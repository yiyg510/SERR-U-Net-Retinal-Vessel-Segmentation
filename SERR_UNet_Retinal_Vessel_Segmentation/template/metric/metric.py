...
class DrawCurve:
	...
	def __init__(self,savepath):
		...
		self.colorbar=['blue','black','red','green']
		self.linestyle=['--','-.','-',':','-*']

	def reload(self,xdata,ydata,auc,dataName,modelName):
		"""
		this function is to update result_auc_stare for Function roc/pr to draw
	...
	def newly(self,modelnum):
		"""
		renew all the result_auc_stare
	...
	def roc(self):
		...
		"""
		plt.figure(1)
		plt.title("ROC Curve of %s with Our U-Net"%self.dataName, fontsize=15)
	...
	def pr(self):
		...
		"""
		plt.figure(2)
		plt.title("LX's PR Curve of %s with SEER-U-Net"%self.dataName, fontsize=15)
